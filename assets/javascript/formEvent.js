    ...
    var typeEventRegister = document.getElementById('type-event-register');
    var peopleEvent = document.getElementById('people-event');
    ...
    var typeEventPayment= document.querySelector('.type-event-payment');
    var peopleEventPayment = document.querySelector('.people-event-payment');
    ...
    let errorcheckin, errorcheckout; 
    var errorPeople;
    var errorDate = {
        'checkin': true,
        'checkout': true
    ...
    function checkoutInput(currentElement) {
        currentElement.onblur = function() {
            ...
            if (checkinRegister.value == '' || checkinRegister.value.includes('bỏ')) {
                ...
                blurStyle(currentElement, 'ngày trả', dateCheckinRegister, errorcheckout);
    ...
    // các xử lí với input số lượng khách
    peopleEvent.onblur = function() {
        if (peopleEvent.value > 700) {
            alert('Sức chứa của sự kiện tối đa là 700 người!');
            peopleEvent.value = 700;
            errorPeople = false;
        } 
        else if (peopleEvent.value < 1) {
            alert('Số người không hợp lệ!')
            peopleEvent.value = 1;
            errorPeople = false;
        }
        if (peopleEvent.value == '') {
            errorStyle(peopleEvent);
            errorPeople = false;
        } else {
            normalStyle(peopleEvent);
            errorPeople = true;
    ...
    function openFormPayment() {
        ...
        if (errorName && errorPhone && errorMail && errorDate.checkin && errorDate.checkout)
        {
            ...
            typeEventPayment.innerHTML = 'Sự kiện: ' + typeEventRegister.value;
            peopleEventPayment.innerHTML = 'Số lượng: ' + peopleEvent.value;
        }
    }
